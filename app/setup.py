...
def run_init():
    ...
    # ── 1.5 Entity type ────────────────────────────────────────────
    click.echo("Entity type:")
    click.echo("  smllc — Single-Member LLC (Schedule C, default)")
    click.echo("  scorp — LLC electing S-Corp status (Form 1120-S)")
    entity_type = click.prompt("  Entity type", default="smllc", type=click.Choice(["smllc", "scorp"], case_sensitive=False))
    ...
    if entity_type == "scorp":
        click.echo()
        click.echo("S-Corp requires a reasonable officer salary.")
        reasonable_salary = _prompt("Annual officer salary ($)", default="50000")
        try:
            reasonable_salary = int(reasonable_salary)
        except ValueError:
            reasonable_salary = 50000
        payroll_frequency = click.prompt(
            "  Payroll frequency",
            default="monthly",
            type=click.Choice(["monthly", "biweekly", "semimonthly"], case_sensitive=False),
        )
    ...
    _write_config(config_path, biz_name, owner, state, ein, address, phone, email,
                  entity_type=entity_type, reasonable_salary=reasonable_salary,
...
def _write_config(path: Path, biz_name: str, owner: str, state: str,
                  ...
                  entity_type: str = "smllc", reasonable_salary: int = 0,
...
[entity]
entity_type = "smllc"
# reasonable_salary = 50000       # Uncomment for S-Corp
# payroll_frequency = "monthly"   # Uncomment for S-Corp
"""
